"""Build 63 market-gate features from SPX/NDX/DJI monthly data."""
import sys
...
def build_index_features(close: pd.Series, dollar_vol: pd.Series, prefix: str) -> pd.DataFrame:
    ...
    amt_ratio = dollar_vol / dollar_vol.replace(0, np.nan)
    feats = {f"{prefix}_ret_1m": ret}
    for d in ROLLING_WINDOWS:
        feats[f"{prefix}_ret_mean_{d}m"] = ret.rolling(d, min_periods=1).mean()
        feats[f"{prefix}_ret_std_{d}m"] = ret.rolling(d, min_periods=1).std().fillna(0)
        roll_amt = dollar_vol.rolling(d, min_periods=1).mean()
        feats[f"{prefix}_amt_mean_ratio_{d}m"] = roll_amt / dollar_vol.replace(0, np.nan)
        roll_std = dollar_vol.rolling(d, min_periods=1).std().fillna(0)
        feats[f"{prefix}_amt_std_ratio_{d}m"] = roll_std / dollar_vol.replace(0, np.nan)
...
def main():
    ...
    print(f"Saved {market.shape[1]} market features x {len(market)} months to {MARKET_FEATURES_CSV}")
